...
async function processInternalTransfer({ sourceAccount, toAccount, transferAmount, description, user_id }) {
  ...
  // Process through double-entry ledger
  const ledgerResult = await DoubleEntryLedger.createTransaction({
    userId: user_id,
  ...
  // Create transaction records (these would reflect the ledger entries)
